...
class BookRepository(BaseRepository[Book, BookCreate, BookUpdate]):
    ...
    def create_with_languages(self, db: Session, *, obj_in: BookCreate) -> Book:
        db_obj = Book(
            title=obj_in.title,
        ...
        for lang_id in obj_in.language_ids:
            stmt = book_language.insert().values(book_id=db_obj.id, language_id=lang_id)
            db.execute(stmt)
    ...
    def update_with_languages(
        ...
        if obj_in.language_ids is not None:
            db.execute(
                book_language.delete().where(book_language.c.book_id == db_obj.id)
            )
    ...
    def get_with_details(self, db: Session, id: int) -> Optional[Book]:
        return (
            db.query(Book)
            .filter(Book.id == id)
            .options(joinedload(Book.chapters), joinedload(Book.languages))
            .first()
        )

    def save_for_user(self, db: Session, *, user_id: int, book_save: BookSave) -> Book:
        db.execute(
        ...
        for lang_id in book_save.language_ids:
            stmt = user_saved_books.insert().values(
                user_id=user_id, book_id=book_save.book_id, language_id=lang_id
            )
            db.execute(stmt)
    ...
    def get_saved_books_for_user(
        ...
        for book, language in query:
            if book.id not in books_map:
                books_map[book.id] = {"book": book, "languages": []}
